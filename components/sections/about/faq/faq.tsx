...
export default function FAQ({faqRef}: { faqRef: RefObject<any> }) {
    return (
        <div className={"min-h-[900px] w-full flex gap-5 flex-col items-center justify-center bgimg bgimg_faq text-white"} ref={faqRef}>
    ...
    <Accordion type="single" collapsible>
        <AccordionItem value="item-1">
            ...
            <AccordionContent className={"text-lg"}>
                {answer} <br/> {optionalLink && <a className={"font-bold underline"} target={"_self"} href={optionalLink}>Click here!</a>}
            </AccordionContent>
        </AccordionItem>
    </Accordion>
    );
